...
// Dynamic host configuration for API calls - RENDER COMPATIBLE
const getApiBaseUrl = () => {
...
app.prepare().then(() => {
  ...
  const server = createServer((req, res) => {
    ...
    if (parsedUrl.pathname === '/debug/rooms' && process.env.NODE_ENV !== 'production') {
  ...
  const io = new Server(server, {
    ...
    // Connection stability settings for Render
    pingTimeout: 60000,
    pingInterval: 25000,
    connectTimeout: 60000,
    maxHttpBufferSize: 10e6,
    transports: ['websocket', 'polling'],
    // Additional Render compatibility
    allowEIO3: true,
    cookie: false
  });
  ...
  server.listen(port, host, (err) => {
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      console.log(`> Socket.IO Network: http://192.168.1.13:${port}`);
